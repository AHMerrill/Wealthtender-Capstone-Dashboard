...
The system operates on pre-built artifacts — no model inference happens at runtime. The full analysis pipeline runs across three notebooks in sequence, producing static CSV/JSON artifacts that the API loads at startup.

### Pipeline Overview

```
Raw Reviews (Wealthtender platform export)
    │
    ▼
┌─────────────────────────────────────────────────────────┐
...
│                                                         │
│  • Filter dates, drop duplicates                        │
│  • Tokenize review text, remove stopwords               │
│  • Compute n-gram frequencies (uni/bi/trigrams)          │
│  • Generate word count distributions                    │
...
│  📓 notebooks/NLP_I.ipynb (Section 2: "Semantic...")    │
│                                                         │
│  • Encode all reviews using all-MiniLM-L6-v2            │
│  • Encode in batches → 384-dim vectors per review       │
│  • Drop advisor names from review text pre-encoding     │
│  • Create review-level and advisor-level embeddings     │
│  • Export: Parquet files in Embeddings Datasets/         │
└─────────────────────────────────────────────────────────┘
    │
    ▼
┌─────────────────────────────────────────────────────────┐
│  Step 3: Dimension Scoring                              │
│  📓 notebooks/collaborator/                             │
│     query_embeddings_vs_review_embeddings.ipynb          │
│                                                         │
│  • Load pre-computed review embeddings                  │
│  • Encode 6 dimension query strings                     │
│  • Compute cosine similarity (review × dimension)       │
│  • Aggregate to entity level: mean, penalized, weighted │
...
|----------|----------|---------|
| **NLP I** | `notebooks/NLP_I.ipynb` | Main analysis notebook: data cleaning, tokenization, stopword removal, n-gram analysis, word count distributions, and sentence-transformer embedding generation. This is the upstream pipeline that produces both EDA artifacts and the review embeddings. |
| **Scoring** | `notebooks/collaborator/query_embeddings_vs_review_embeddings.ipynb` | Dimension scoring: loads pre-computed embeddings, encodes 6 dimension query strings, computes cosine similarity, aggregates to entity level using three methods, exports scoring CSVs. |
| **Scoring Exploration** | `notebooks/Scoring_Exploration.ipynb` | Experimental notebook exploring alternative scoring approaches (KMeans clustering, cosine similarity experiments). Not part of the production pipeline. |

### EDA Artifacts (`artifacts/macro_insights/`)
Generated by NLP_I.ipynb (Steps 1.1–1.6). The API loads these into memory on startup.
...
Generated by the scoring notebook (`notebooks/collaborator/query_embeddings_vs_review_embeddings.ipynb`):

| File | Contents |
|------|----------|
| `review_dimension_scores.csv` | Per-review cosine similarity scores for 6 dimensions |
...
To regenerate: run NLP_I.ipynb end-to-end (produces embeddings), then run the scoring notebook (produces CSVs), then copy outputs to `artifacts/scoring/` and rebuild.
...
The current system is snapshot-based — artifacts are static files baked into the API Docker image at build time. To update the data, you rerun the notebooks, replace the CSVs, and rebuild the container.
